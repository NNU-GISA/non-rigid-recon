...
typedef std::vector<TRIANGLE1> TRIANGLEVECTOR;
...
public:
	...
	CIsoSurface();
	~CIsoSurface();
	typedef std::vector<Eigen::Matrix<T, 3, 1>> outPt_container;
	...
	void SaveMeshPly(const std::string &filePath, const Eigen::Matrix<T, 3, 1>  &offset);
	void SaveMeshPlyWithNormal(const std::string &filePath, const Eigen::Matrix<T, 3, 1> &offset);
	void GetMeshPointsWithNormals(outPt_container &pt_clouds, outPt_container &pt_normals, const Eigen::Vector3f &offset);
